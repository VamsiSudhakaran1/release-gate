...
def _commit_file(
    ...
    try:
        existing = _gh("GET", f"/repos/{owner}/{repo}/contents/{path}?ref={branch}", token)
        file_sha: Optional[str] = existing.get("sha")
    except RuntimeError:
        file_sha = None
    payload: Dict[str, Any] = {"message": message, "content": encoded, "branch": branch}
    if file_sha:
        payload["sha"] = file_sha
    _gh("PUT", f"/repos/{owner}/{repo}/contents/{path}", token, payload)


def create_fix_pr(
    owner: str, repo: str, report: Dict, installation_token: str,
) -> str:
    """Open a PR with auto-generated governance files for missing safeguards.

    Only commits files for safeguards that are actually missing — nothing extra.
    Returns the HTML URL of the opened (or already-open) PR.
    """
    import sys, os
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
    from release_gate.audit import emit_config, emit_evals

    token = installation_token
    branch_name = "release-gate/auto-fix"

    # Resolve default branch and its HEAD SHA
    repo_data = _gh("GET", f"/repos/{owner}/{repo}", token)
    ...
    def _present(key: str) -> bool:
        v = safeguards.get(key, False)
        return v if isinstance(v, bool) else bool(v.get("present", False))

    files_created: list[str] = []

    # governance.yaml covers: governance_file, budget_ceiling, kill_switch,
    # team_owner, auth_rate_limit, trace_policy
    governance_needed = not _present("governance_file")
    if governance_needed:
        _commit_file(
            token, owner, repo, branch_name,
            "governance.yaml",
            emit_config(report),
            "chore: add governance.yaml (release-gate auto-fix)",
        )
        files_created.append("governance.yaml")

    # evals.yaml covers: eval_evidence
    if not _present("eval_evidence"):
        _commit_file(
            token, owner, repo, branch_name,
            "evals.yaml",
            emit_evals(report),
            "chore: add evals.yaml scaffold (release-gate auto-fix)",
        )
        files_created.append("evals.yaml")

    if not files_created:
        # Nothing to fix — all safeguards present
        raise RuntimeError("No missing safeguards to fix — repo already has all required files.")
    ...
    todo_items = []
    if "governance.yaml" in files_created:
        todo_items += [
            "[ ] Set your real model ID (search for `# TODO: set your real model id`)",
            "[ ] Set your daily cost ceiling (`max_daily_cost`)",
            "[ ] Set the team owner and runbook URL in `fallback_declared`",
            "[ ] Update `allowed_tools` / `forbidden_tools` to match your agent's actual tool list",
        ]
    if "evals.yaml" in files_created:
        todo_items += [
            "[ ] Replace placeholder expected phrases in `evals.yaml` with real outputs from your agent",
            "[ ] Add domain-specific test cases (see the TODO section at the bottom of `evals.yaml`)",
        ]
    todos_md = "\n".join(todo_items)

    pr_body = (
        f"## release-gate auto-fix\n\n"
        f"This PR adds the missing governance files identified by release-gate.\n\n"
        f"**Score before fix:** {score}/100 ({decision})\n\n"
        f"**Files added:**\n{files_md}\n\n"
        f"---\n\n"
        f"### Before you merge — fill in the `# TODO:` lines\n\n"
        f"{todos_md}\n\n"
        f"After merging, re-scan at [release-gate.com](https://release-gate.com) "
        f"to confirm your score improved.\n\n"
        f"<sub>Generated by [release-gate](https://release-gate.com) · "
        f"[What is release-gate?](https://github.com/VamsiSudhakaran1/release-gate)</sub>"
    )
    ...
    try:
        ...
        # PR already open — retrieve its URL
        try:
            prs = _gh("GET",
