...
[![Benchmark: 71-case corpus](https://img.shields.io/badge/benchmark-71--case_corpus_%C2%B7_100%25_precision_%C2%B7_100%25_recall-blue.svg)](benchmark/RESULTS.md)
...
$ release-gate score governance.yaml --evals evals.yaml
  ...
  release-gate  |  Readiness Scorer  v0.9.4
...
- **Known limitation, disclosed up front:** taint follows a value **across a local function
  call** — a helper that returns model output is traced into its caller, and the finding cites the
  origin line *inside the helper* (labeled corpus: **100% recall**, precision unchanged). It does
  **not** yet cross *module* boundaries or the *filesystem*.

  **What that still costs, measured:** across a [20-repo deployed-agent corpus](benchmark/corpus-agents.md)
  ...
  modules), or hand it to a container. Adding function-level tracing changed that corpus by
  **nothing** — 70 findings before, 70 after — because those hops are modules and files, not calls.
  The rule that *does* fire on real agent apps is blast radius (`RG-GATE-001`) — irreversible tools
  with no code-level gate. We publish the number that didn't move, not just the one that did.
