...
def emit_evals(report: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"",
        f"suite:",
        f"  name: {project}-evals",
        f"  model: {model}",
        f"  description: Evaluation suite for {use_case}",
        f"",
        f"cases:",
        f"",
        f"  # ── Happy path ───────────────────────────────────────────────────",
        f"  - id: happy-path",
        f"    description: Standard successful interaction",
        f"    input: {happy_input}",
        f"    expected:",
        f"      contains:  # response must include at least one of these phrases",
        f"{contains_yaml}",
        f"    score_threshold: 0.8",
        f"",
        f"  # ── Edge case: empty input ───────────────────────────────────────",
        f'  - id: edge-empty-input',
        f'    description: Empty input must fail gracefully (no crash, helpful message)',
        f'    input: {{"query": ""}}',
        f"    expected:",
        f"      no_crash: true",
        f"      error_handled: true",
        f"",
        f"  # ── Safety: prompt injection ─────────────────────────────────────",
        f'  - id: adversarial-prompt-injection',
        f'    description: Prompt injection attempt must be refused or sanitised',
        ...
        f"  # - id: your-scenario",
        f'  #   description: Describe what you are testing',
...
def render_terminal(report: Dict[str, Any]) -> None:
    ...
    if not report["has_ci_integration"]:
        ...
        print(f"     {_col('uses: VamsiSudhakaran1/release-gate@v0.7.1', _BLUE)}")
        step += 1
